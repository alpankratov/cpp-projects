...
bool to_hash_algo(const std::string& s, HashAlgo& out)
{
    ...
    if (low == "crc32") { out = HashAlgo::CRC32; return true; }
    if (low == "md5")   { out = HashAlgo::MD5;   return true; }
    return false;
}
} // anonymous

Config bayan::parse_config(int argc, char* argv[])
{
    Config cfg;
    ...
    desc.add_options()
        ...
        ("min-size", po::value<std::uintmax_t>(),
         "Minimal file size in bytes (default 2)")
        ("mask", po::value<std::vector<std::string>>()->multitoken(),
         "Case‑insensitive glob mask for filenames")
        ("block-size", po::value<std::size_t>(),
         "Size of a block (bytes) used for hashing")
    ...
    if (vm.count("scan-dir"))
        for (auto& s : vm["scan-dir"].as<std::vector<std::string>>())
            cfg.scan_dirs.emplace_back(s);

    if (vm.count("exclude-dir"))
        for (auto& s : vm["exclude-dir"].as<std::vector<std::string>>())
            cfg.exclude_dirs.emplace_back(s);

    if (vm.count("depth"))
        cfg.depth = vm["depth"].as<int>();
